...
export default function AuthorDetail() {
  ...
  const { error, setError, loading, setLoading } = useStory();
  ...
  return (
    <>
      ...
      {error && <p className="alert alert-secondary text-center w-50 mx-auto m-3">{error}</p>}
                  ...
                  <div className="col col-9">
                    <Link to={`/short-stories/${story?.slug}`}>
                      <div className="d-flex gap-3 align-items-center">
                        ...
                        <div className="author_story_title">{story?.title}</div>
                      </div>
                    </ Link>
                  </div>

                  <div className="col col-2 align-items-center text-end mt-1">
                    <Link to={`/issues/issue/${story?.issue?.pubblication_number}`}><small>Issue {story?.issue?.pubblication_number}</small></ Link>
                  </div>

                  <div className="col author_story_tags d-flex gap-3 mt-1">
